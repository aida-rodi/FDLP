...
// EJERCICIO RESUELTO CON MÉTODOS DE ARRAY

let foundFilms = [...films];

function showFilms(films) {
    const formatedFilms = films.map((film, i) => {
        return `🎥 <strong>${i}. ${film.title}</strong> <br> 
            Year: ${film.year} <br> 
            Genre: ${film.genre.join(", ")} <br>
            Director: ${film.director} <br> 
            Actors: ${film.actors.join(", ")} <br> 
...
function searchFilmsByCriteria() {
    ...
    foundFilms = films.filter(function (film) {
        ...
        return ( existsInTitle || existsInYear || existsInGenre || existsInDirector || existsInActors);
    });
...
function findByGenre(genre) {
    foundFilms = films.filter((film) => film.genre.includes(genre));
